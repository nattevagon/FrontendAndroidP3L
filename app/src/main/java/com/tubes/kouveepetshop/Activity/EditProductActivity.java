...
public class EditProductActivity extends AppCompatActivity {
    ...
    private String sId, sName, sPrice, sStock, sMinimal, sUnit, sImage, url, imagePath;
    private int posUnit;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sStock = i.getStringExtra("stock");
        sMinimal = i.getStringExtra("minimal");
        sUnit = i.getStringExtra("unit");
        sImage = i.getStringExtra("image");

        etName.setText(sName);
        etPrice.setText(sPrice);
        etStock.setText(sStock);
        etMinimal.setText(sMinimal);
        posUnit = getIndex(spUnit, sUnit);
        spUnit.setSelection(posUnit);
        url = "https://kouvee.modifierisme.com/upload/"+sImage;
        Picasso.with(EditProductActivity.this).load(url).resize(300,300).centerCrop().into(imgProduct);
    ...
    private void Edit(String image) {
        ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
        Call<ProductDAO> update = apiService.updateProduct(sId, etName.getText().toString(),etPrice.getText().toString()
